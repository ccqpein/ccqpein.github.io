...
I have done some benchmarks inside my demo repo. However, I think it was years ago and I need re-write some new benchmarks. After some recall and study, I check the `criterion` lib and it is good enough so I change it from `[bench]` to `criterion`.

After get the benchmark result, I was thinking if I would like use flamegraph. I tried use `pprof` for generating flamegraph with `criterion`, but it didn't go well. Then I find the [flamegraph](https://github.com/flamegraph-rs/flamegraph). After solve the `SIP` issue on Apple Silicon by `csrutil enable --without dtrace` in recovery mode.

After all, run `cargo flamegraph -o bench0_flamegraph --bench bench0 -- --bench` and generated the flamegraph then I can check which part cost a lot time.

### Memory allocation ###

After flamegraph generated, I noticed that the memory allocation cause a lot time. So I just do a little bit optimization: make all function add string to `&mut string` buffer rather than return the totally new string. It increasing the speed almost 20% immediatly to all benchmark cases. And of course, with some prediction of memory use by `String::with_capacity(args.left_count());` to generat the string buffer.
...
+ allocation memory truly cost a lot time.
